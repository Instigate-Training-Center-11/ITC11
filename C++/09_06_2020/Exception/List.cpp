...
namespace Data {
    ...
    template <typename T>
    void LinkedList<T>::insertElement(T value, int number) {
        if (length < number) {
            throw MyException("Can not added element in that index!");
        }
    ...
    template <typename T>
    void LinkedList<T>::removeElement(int number) {
        if (length < number) {
            throw MyException("Can not delete that element!");
        }
    ...
    template <typename T>
    T LinkedList<T>::getElement(int number) {
        if (length >= number) {
            throw MyException("Can not get that element!");
        }
        int count = 0;
        /*
